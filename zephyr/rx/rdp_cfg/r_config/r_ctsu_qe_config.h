...
#else
#define CTSU_CFG_NUM_SELF_ELEMENTS                (DT_PROP(DT_NODELABEL(ctsu), max_num_sensors))
#define CTSU_CFG_NUM_MUTUAL_ELEMENTS              (0)
#endif
#define CTSU_CFG_LOW_VOLTAGE_MODE                 (DT_PROP(DT_NODELABEL(ctsu), low_voltage_mode))
#define CTSU_CFG_PCLK_DIVISION                    (CONFIG_INPUT_RENESAS_RX_CTSU_PCLK_DIVISION)
#define CTSU_CFG_TSCAP_PORT                                                                        \
	(DT_PROP(DT_GPIO_CTLR(DT_INST(0, renesas_rx_ctsu), tscap_gpios), port) << 8 |               \
	 DT_GPIO_PIN(DT_INST(0, renesas_ra_ctsu), tscap_gpios))
