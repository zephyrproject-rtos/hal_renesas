...
*/
#define BSP_CFG_MAIN_CLOCK_OSCILLATE_ENABLE    (DT_NODE_HAS_STATUS(DT_NODELABEL(xtal), okay))
...
*/
#define BSP_CFG_SUB_CLOCK_OSCILLATE_ENABLE     (DT_NODE_HAS_STATUS(DT_NODELABEL(subclk), okay))
...
*/
#define BSP_CFG_HOCO_OSCILLATE_ENABLE          (DT_NODE_HAS_STATUS(DT_NODELABEL(hoco), okay))
...
*/
#define BSP_CFG_LOCO_OSCILLATE_ENABLE          (DT_NODE_HAS_STATUS(DT_NODELABEL(loco), okay))
...
*/
#define BSP_CFG_IWDT_CLOCK_OSCILLATE_ENABLE    (DT_NODE_HAS_STATUS(DT_NODELABEL(iwdtlsclk), okay))
...
/* Clock source select (CKSEL).
   ...
   4 = PLL Circuit
*/
#define BSP_CFG_CLOCK_SOURCE            (RX_CGC_CLK_SRC(DT_CLOCKS_CTLR(DT_NODELABEL(pclkblock))))
...
*/
#define BSP_CFG_USB_CLOCK_SOURCE        (RX_IF_CLK_SRC(DT_CLOCKS_CTLR(DT_NODELABEL(uclk))))
...
/* CANFD Clock source select (CANFDCKSEL). Choose which clock source to input to the CANFD circuit.
   ...
   6 = PLL 2 Circuit
*/
#define BSP_CFG_CANFD_CLOCK_SOURCE      (RX_IF_CLK_SRC(DT_CLOCKS_CTLR(DT_NODELABEL(canfdclk))))
...
*/
#define BSP_CFG_LPT_CLOCK_SOURCE        (RX_LPT_CLK_SRC(DT_CLOCKS_CTLR(DT_NODELABEL(lptclk))))

/* Main clock Oscillator Switching (MOSEL).
   0 = Resonator
   1 = External clock input
*/
#define BSP_CFG_MAIN_CLOCK_SOURCE       (RX_CGC_PROP_HAS_STATUS_OKAY_OR(DT_NODELABEL(xtal), mosel, 0))

/* Sub clock Oscillator Switching (SOSEL).
   0 = Resonator
   1 = External clock input
*/
#define BSP_CFG_SUB_CLOCK_SOURCE        (RX_CGC_PROP_HAS_STATUS_OKAY_OR(DT_NODELABEL(subclk), mosel, 0))
 ...
 */
#define BSP_CFG_CLKOUT_SOURCE           (RX_CGC_CLK_SRC(DT_CLOCKS_CTLR(DT_NODELABEL(clkout))))
...
*/
#define BSP_CFG_RTC_ENABLE              (DT_NODE_HAS_STATUS(DT_NODELABEL(rtcsclk), okay))
...
*/
#define BSP_CFG_SOSC_DRV_CAP            (RX_CGC_PROP_HAS_STATUS_OKAY_OR(DT_NODELABEL(subclk),  drive_capacity, 0))

/* USB Clock supply control (USBCKSREQ).
   0 = Stop clock supply to USB.
   1 = Supply clock to USB. (default)
*/
#define BSP_CFG_USB_CLOCK_ENABLE        (DT_NODE_HAS_STATUS(DT_NODELABEL(uclk), okay))

/* CANFD Clock supply control (CANFDCKSREQ).
   0 = Stop clock supply to CANFD.
   1 = Supply clock to CANFD. (default)
*/
#define BSP_CFG_CANFD_CLOCK_ENABLE      (DT_NODE_HAS_STATUS(DT_NODELABEL(canfdclk), okay))
...
#define BSP_CFG_XTAL_HZ                 (RX_CGC_PROP_HAS_STATUS_OKAY_OR(DT_NODELABEL(xtal), clock_frequency, 0))

/* PLL Input Frequency Division Ratio Select (PLIDIV).
   Available divisors = /1 (no division), /2, /4
*/
#define BSP_CFG_PLL_DIV                 (RX_CGC_PROP_HAS_STATUS_OKAY_OR(DT_NODELABEL(pll), div, 2))
...
/* PLL2 Input Frequency Division Ratio Select (PLIDIV).
   Available divisors = /1 (no division), /2, /4
*/
#define BSP_CFG_PLL2_DIV                (RX_CGC_PROP_HAS_STATUS_OKAY_OR(DT_NODELABEL(pll2), div, 2))

/* PLL2 Frequency Multiplication Factor Select (STC).
   Available multipliers = x4.0 to x15.5 in 0.5 increments (e.g. 4.0, 4.5, 5.0, ..., 14.5, 15.0, 15.5)
*/
#define BSP_CFG_PLL2_MUL                ((RX_CGC_PROP_HAS_STATUS_OKAY_OR(DT_NODELABEL(pll2), mul, 15) + 1) / (2.0))

/* System Clock Divider (ICK).
   Available divisors = /1 (no division), /2, /4, /8, /16, /32, /64
*/
#define BSP_CFG_ICK_DIV                 (RX_CGC_PROP_HAS_STATUS_OKAY_OR(DT_NODELABEL(iclk), div, 1))

/* Peripheral Module Clock A Divider (PCKA).
   Available divisors = /1 (no division), /2, /4, /8, /16, /32, /64
*/
#define BSP_CFG_PCKA_DIV                (RX_CGC_PROP_HAS_STATUS_OKAY_OR(DT_NODELABEL(pclka), div, 1))

/* Peripheral Module Clock B Divider (PCKB).
   Available divisors = /1 (no division), /2, /4, /8, /16, /32, /64
*/
#define BSP_CFG_PCKB_DIV                (RX_CGC_PROP_HAS_STATUS_OKAY_OR(DT_NODELABEL(pclkb), div, 1))

/* Peripheral Module Clock D Divider (PCKD).
   Available divisors = /1 (no division), /2, /4, /8, /16, /32, /64
*/
#define BSP_CFG_PCKD_DIV                (RX_CGC_PROP_HAS_STATUS_OKAY_OR(DT_NODELABEL(pclkd), div, 1))

/* Flash IF Clock Divider (FCK).
   Available divisors = /1 (no division), /2, /4, /8, /16, /32, /64
*/
#define BSP_CFG_FCK_DIV                 (RX_CGC_PROP_HAS_STATUS_OKAY_OR(DT_NODELABEL(fclk), div, 1))
...
/* CLKOUT Output Frequency Division Ratio Select. (CKODIV)
   ...
   7 = x1/128
 */
#define BSP_CFG_CLKOUT_DIV              (RX_CGC_PROP_HAS_STATUS_OKAY_OR(DT_NODELABEL(clkout), div, 1))
 ...
 */
#define BSP_CFG_CLKOUT_OUTPUT           (DT_NODE_HAS_STATUS(DT_NODELABEL(clkout), okay))
...
*/
#define BSP_CFG_MOSC_WAIT_TIME          (RX_CGC_PROP_HAS_STATUS_OKAY_OR(DT_NODELABEL(xtal), stabilization_time, 4))

/* Sub-Clock Oscillator Wait Time (Use R_BSP_SoftwareDelay).
   Setting delay unit is in milliseconds.
*/
#define BSP_CFG_SOSC_WAIT_TIME          (RX_CGC_PROP_HAS_STATUS_OKAY_OR(DT_NODELABEL(subclk), sub_clk_osc, 1482))
...
*/
#define BSP_CFG_OFS1_REG_VALUE  (SET_OFS1_HOCO_BITS(0xFFFFFFFF, (RX_CGC_PROP_HAS_STATUS_OKAY_OR(DT_NODELABEL(hoco), clock_frequency, 64000000))))
