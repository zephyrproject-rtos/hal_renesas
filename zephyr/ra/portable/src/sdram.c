 ...
 * e.g. tREF = 64ms/8192rows -> 7.8125us/each row.  937cycles are needed at
 * SDCLK 120MHz
 */
#define BSP_PRV_SDRAM_REF_CMD_INTERVAL         (937U)

/*
 * Set Auto-Refresh issue times in initialization sequence needed for SDRAM
 * device Typical SDR SDRAM device needs twice of Auto-Refresh command issue
 */
...
#if ((BSP_PRV_SDRAM_SDADR_ROW_ADDR_OFFSET != 8U) &&                            \
    (BSP_PRV_SDRAM_SDADR_ROW_ADDR_OFFSET != 9U) &&                            \
    (BSP_PRV_SDRAM_SDADR_ROW_ADDR_OFFSET != 10U) &&                           \
    (BSP_PRV_SDRAM_SDADR_ROW_ADDR_OFFSET > 11U))
 #error "BSP_PRV_SDRAM_SDADR_ROW_ADDR_OFFSET must be either of 8,9,10 or 11"
#endif

#if ((BSP_PRV_SDRAM_BUS_WIDTH != 0) && (BSP_PRV_SDRAM_BUS_WIDTH != 1U) &&      \
    (BSP_PRV_SDRAM_BUS_WIDTH != 2U))
 #error                                                                         \
    "BSP_PRV_SDRAM_BUS_WIDTH must be either of 0(16-bit) or 1(32-bit) or 2(8-bit)"
#endif
...
#if ((BSP_PRV_SDRAM_CONTINUOUS_ACCESSMODE != 0) &&                             \
    (BSP_PRV_SDRAM_CONTINUOUS_ACCESSMODE != 1))
 #error                                                                         \
...
#define BSP_PRV_SDRAM_MR_WB_SINGLE_LOC_ACC                                     \
        (1U)                           /* MR.M9                : Single Location Access */
#define BSP_PRV_SDRAM_MR_OP_MODE                                               \
        (0U)                           /* MR.M8:M7             : Standard Operation */
#define BSP_PRV_SDRAM_MR_BT_SEQUENCTIAL                                        \
        (0U)                           /* MR.M3 Burst Type     : Sequential */
#define BSP_PRV_SDRAM_MR_BURST_LENGTH                                          \
        (0U)                           /* MR.M2:M0 Burst Length: 0(1 burst) */
...
void sdram_init (void) {
    ...
    {
        /* According to h/w maual, need to confirm that all the status bits in SDSR
         * are 0 before SDIR modification. */
    }
    ...
    {
        /* According to h/w maual, need to confirm that all the status bits in SDSR
         * are 0 before SDICR modification. */
    }
    ...
    R_BUS->SDRAM.SDCCR_b.BSIZE =
        BSP_PRV_SDRAM_BUS_WIDTH;             /* set SDRAM bus width */
    R_BUS->SDRAM.SDAMOD_b.BE =
        BSP_PRV_SDRAM_CONTINUOUS_ACCESSMODE; /* enable continuous access */
    R_BUS->SDRAM.SDCMOD_b.EMODE =
        BSP_PRV_SDRAM_ENDIAN_MODE;           /* set endian mode for SDRAM address space */

    while (R_BUS->SDRAM.SDSR)
    {
        /* According to h/w maual, need to confirm that all the status bits in SDSR
         * are 0 before SDMOD modification. */
    }
    ...
    R_BUS->SDRAM.SDTR_b.RAS =
        BSP_PRV_SDRAM_TRAS - 1U;       /* set ACTIVE-to-PRECHARGE command cycles*/
    R_BUS->SDRAM.SDTR_b.RCD =
        BSP_PRV_SDRAM_TRCD - 1U;       /* set ACTIVE to READ/WRITE delay cycles */
    R_BUS->SDRAM.SDTR_b.RP =
        BSP_PRV_SDRAM_TRP - 1U;        /* set PRECHARGE command period cycles */
    R_BUS->SDRAM.SDTR_b.WR =
        BSP_PRV_SDRAM_TWR - 1U;        /* set write recovery cycles */
    R_BUS->SDRAM.SDTR_b.CL =
        BSP_PRV_SDRAM_CL;              /* set SDRAM column latency cycles */

    /** Set row address offset for target SDRAM */
    R_BUS->SDRAM.SDADR_b.MXC = BSP_PRV_SDRAM_SDADR_ROW_ADDR_OFFSET - 8U;

    R_BUS->SDRAM.SDRFCR_b.REFW =
        (uint16_t) (BSP_PRV_SDRAM_TRFC - 1U); /* set Auto-Refresh issuing cycle */
    R_BUS->SDRAM.SDRFCR_b.RFC =
        BSP_PRV_SDRAM_REF_CMD_INTERVAL - 1U;  /* set Auto-Refresh period */
