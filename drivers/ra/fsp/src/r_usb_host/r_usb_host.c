...
fsp_err_t R_USBH_PortReset (usb_ctrl_t * const p_api_ctrl)
{
    ...
    g_uhc_data[p_ctrl->p_cfg->module_number].need_reset = false;
                                                      ...
                                                      usb_xfer_result_t            result)
{
    usb_event_t event =
    {
        .event_id = USB_EVENT_XFER_COMPLETE,
        .dev_addr = dev_addr,
    };
    event.ep_addr = ep_addr;
    event.result  = result;
    event.len     = xferred_bytes;

    usb_callback_arg_t   args;
    usb_callback_arg_t * p_args = p_ctrl->p_callback_memory;
    ...
    memcpy(&p_args->event, &event, sizeof(usb_event_t));
...
{
    usb_event_t event;
    event.event_id = USB_EVENT_DEVICE_ATTACH;
    event.hub_addr = 0;
    event.hub_port = 0;
...
{
    usb_event_t event;
    event.event_id = USB_EVENT_DEVICE_REMOVE;
    event.hub_addr = 0;
    event.hub_port = 0;
...
void r_usbh_isr (void)
{
    ...
    {
        p_reg->DVSTCTR0_b.UACT = 1;
        g_uhc_data[p_ctrl->p_cfg->module_number].need_reset = true;
        p_reg->INTENB1 =
