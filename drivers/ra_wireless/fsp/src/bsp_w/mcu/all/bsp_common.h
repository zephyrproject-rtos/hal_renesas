  ...
  #define BSP_GET_SP()                                                                     \
    ({                                                                                     \
        ...
        __ASM volatile (                                                                   \
            ...
            :[bsp_ret] "=r" (bsp_ret)  /* Output operands: Store the SP value on bsp_ret */\
            :                          /* Input operands: None */                          \
            :                          /* Clobbers: None */                                \
            );                                                                             \
        bsp_ret;                                                                           \
...
typedef enum e_fsp_priv_clock
{
    ...
    FSP_PRIV_CLOCK_PSK,
    FSP_PRIV_CLOCK_TDES,
    FSP_PRIV_CLOCK_SYS_MEM,
    FSP_PRIV_CLOCK_DMA,
    FSP_PRIV_CLOCK_KDMA,
    FSP_PRIV_CLOCK_CC312,

    /* QSPI Div */
    FSP_PRIV_CLOCK_OQSPI,
    FSP_PRIV_CLOCK_QSPI,

    /* SPI Div */
    FSP_PRIV_CLOCK_SPI,
    FSP_PRIV_CLOCK_SPI2,
    FSP_PRIV_CLOCK_TIMER,
    FSP_PRIV_CLOCK_TIMER2,
    FSP_PRIV_CLOCK_TIMER3,
    FSP_PRIV_CLOCK_TIMER4,
    FSP_PRIV_CLOCK_TIMER5,
    FSP_PRIV_CLOCK_TIMER6,
    FSP_PRIV_CLOCK_TIMER7,
    FSP_PRIV_CLOCK_TIMER8,
    FSP_PRIV_CLOCK_I2C,
    FSP_PRIV_CLOCK_I2C2,
    FSP_PRIV_CLOCK_SDIO,
    FSP_PRIV_CLOCK_SDEMMC,
    FSP_PRIV_CLOCK_UART,
    FSP_PRIV_CLOCK_AUX_ADC,
    FSP_PRIV_CLOCK_PLL_PHY,
    FSP_PRIV_CLOCK_WDT
#endif
} fsp_priv_clock_t;
...
__STATIC_INLINE uint32_t R_FSP_SystemClockHzGet (fsp_priv_clock_t clock)
{
    ...
    return R_BSP_PeripheralClockHzGet(clock);
#else
...
/*******************************************************************************************************************//**
 * Safe string copy: copies up to (dst_size - 1) characters from src to dst
 * and always NUL-terminates dst. Has no effect if dst_size is 0.
 *
 ...
 **********************************************************************************************************************/
__STATIC_INLINE void bsp_safe_strcpy (char * dst, const char * src, size_t dst_size)
{
    if (dst_size == 0)
    {
        return;
