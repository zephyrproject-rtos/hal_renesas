...
fsp_err_t R_INTC_IRQ_ExternalIrqOpen (external_irq_ctrl_t * const p_api_ctrl, external_irq_cfg_t const * const p_cfg)
{
    ...
    if ((p_cfg->channel >= BSP_FEATURE_INTC_IRQ_INTTSEL_MIN_CHANNEL) &&
        (p_cfg->channel <= BSP_FEATURE_INTC_IRQ_INTTSEL_MAX_CHANNEL))
    {
        R_BSP_IntCauseSelectionSet(FSP_IP_IRQ, p_cfg->channel, INTSEL_CAUSE_IRQ);
    }
#endif
