...
open(my $header, '>', $headername) or die("Can not open output file");

binmode $out;
binmode $header;
# Write header
$header->print(pack('L', $size));
for(my $i = 1; $i < 4; $i++) {
    $header->print(pack('L', 0xffffffff));
}
$header->print($digest);
for(my $i = 12; $i < 112; $i++) {
	$header->print(pack('L', 0xffffffff));
}
$header->print(pack('L', 0xffff0000));
$header->print(pack('L', 0x000800ff));
$header->print(pack('L', 0x00fff700));
$header->print(pack('L', 0xf700f708));
$header->print(pack('L', 0xffffffff));
$header->print(pack('L', 0xffffffff));
$header->print(pack('L', 0xffffffff));
$header->print(pack('L', 0xffffffff));
...
$header->print(pack('L', 0xffffffff));
$header->print(pack('L', 0xffffffff));
$header->print(pack('L', 0xaa55ffff));
close($header);
open($header, '<', $headername) or die("Can not open output file");
binmode $header;

read($header, my $hbuf, 0x200);
my @head = unpack('C512', $hbuf);

for(my $h = 0;$h<2;$h++) {
    for my $i (0..$#head){
        $out->print(pack('C', $head[$i]));
    }
    for my $i (0..$#head){
        $out->print(pack('C', 255 - $head[$i]));
    }
}
