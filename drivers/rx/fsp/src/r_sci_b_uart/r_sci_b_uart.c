...
#define SCI_B_UART_SCR2_DEFAULT_VALUE          (0xFF00FF04U)
#define SCI_B_UART_FCR_DEFAULT_VALUE           (0x1F1F0000U)

/* SCI SCR1 register bit offsets and masks */
#define SCI_B_UART_SCR1_SPB2_OFFSET            (4U)
#define SCI_B_UART_SCR1_SPB2_MASK              (0x00000030U)
#define SCI_B_UART_SCR1_PARITY_OFFSET          (8U)
#define SCI_B_UART_SCR1_PARITY_MASK            (0x00000300U)
#define SCI_B_UART_SCR1_FLOW_CTSRTS_MASK       (0x00000003U)

/* SCI SCR3 register bit offsets and masks */
#define SCI_B_UART_SCR3_CHAR_OFFSET            (8U)
#define SCI_B_UART_SCR3_CHAR_MASK              (0x00000300U)
#define SCI_B_UART_SCR3_CKE_OFFSET             (24U)
#define SCI_B_UART_SCR3_CKE_MASK               (0x03000000U)

/* SCI Data register bit masks */
#define SCI_B_UART_TDR_TDAT_MASK_9BITS         (0x0000FDFFU)

/* SCI SSR register receiver error (overflow, framing, parity) bits masks */
#define SCI_B_UART_RCVR_ERR_MASK               (R_SCI_B0_SSR_ORER_Msk | R_SCI_B0_SSR_AFER_Msk | R_SCI_B0_SSR_APER_Msk)

/* SCI SSCR register bit masks */
#define SCI_B_UART_SSCR_CLEAR_ALL_MASK         (0x9D070010U)
#define SCI_B_UART_SSCR_RDRFC_MASK             (0x80000000U)
#define SCI_B_UART_SSCR_TDREC_MASK             (0x20000000U)

/* SCI RFSCR register bit masks */
#define SCI_B_UART_RFSCR_CLEAR_ALL_MASK        (0x00000001U)
...
#define SCI_B_UART_DTC_RX_TRANSFER_SETTINGS    (((uint32_t) TRANSFER_MODE_NORMAL << TRANSFER_SETTINGS_MODE_BITS) | \
...
#define SCI_B_UART_DTC_TX_TRANSFER_SETTINGS    (((uint32_t) TRANSFER_MODE_NORMAL << TRANSFER_SETTINGS_MODE_BITS) | \
                                                ((uint32_t) TRANSFER_SIZE_1_BYTE << TRANSFER_SETTINGS_SIZE_BITS) | \
                                                ((uint32_t) TRANSFER_ADDR_MODE_INCREMENTED <<                      \
                                                 TRANSFER_SETTINGS_SRC_ADDR_BITS) |                                \
                                                ((uint32_t) TRANSFER_IRQ_END << TRANSFER_SETTINGS_IRQ_BITS) |      \
...
fsp_err_t R_SCI_B_UART_Open (uart_ctrl_t * const p_api_ctrl, uart_cfg_t const * const p_cfg)
{
    ...
    uint32_t scr0 = R_SCI_B0_SCR0_IDSEL_Msk;
    ...
    /* Clear all flags in SSR register. */
    p_ctrl->p_reg->SSCR = SCI_B_UART_SSCR_CLEAR_ALL_MASK;

#if SCI_B_UART_CFG_FIFO_SUPPORT
    p_ctrl->p_reg->RFSCR = SCI_B_UART_SSCR_CLEAR_ALL_MASK;
#endif

#if (SCI_B_UART_CFG_RX_ENABLE)

    /* If reception is enabled at build time, enable reception. */
    scr0 |= R_SCI_B0_SCR0_RE_Msk;
    R_BSP_IrqEnable(p_ctrl->p_cfg->rxi_irq);
    R_BSP_IrqEnable(p_ctrl->p_cfg->eri_irq);

    scr0 |= R_SCI_B0_SCR0_RIE_Msk;
#endif
    ...
    FSP_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->SISSR_b.IREF, 1U);
#else
...
#if (SCI_B_UART_CFG_TX_ENABLE)
    p_ctrl->p_reg->SCR0 &= (uint32_t) ~(R_SCI_B0_SCR0_TIE_Msk | R_SCI_B0_SCR0_TEIE_Msk);
...
#if (SCI_B_UART_CFG_RX_ENABLE)
    p_ctrl->p_reg->SCR0 &= (uint32_t) ~(R_SCI_B0_SCR0_RIE_Msk);
...
fsp_err_t R_SCI_B_UART_Write (uart_ctrl_t * const p_api_ctrl, uint8_t const * const p_src, uint32_t const bytes)
{
     ...
     * hardware manual. */
    p_ctrl->p_reg->SCR0 |= (uint32_t) (R_SCI_B0_SCR0_TE_Msk | R_SCI_B0_SCR0_TIE_Msk);
...
fsp_err_t R_SCI_B_UART_BaudSet (uart_ctrl_t * const p_api_ctrl, void const * const p_baud_setting)
{
    ...
    FSP_ASSERT((p_ctrl->p_reg->SCR3_b.CKE & 0x2) == 0U);
#endif

    /* Save SCR configurations except transmit interrupts. Resuming transmission after reconfiguring baud settings is
     * not supported. */
    uint32_t preserved_scr0 = p_ctrl->p_reg->SCR0 & (uint32_t) ~(R_SCI_B0_SCR0_TIE_Msk | R_SCI_B0_SCR0_TEIE_Msk);

    /* Disables transmitter and receiver. This terminates any in-progress transmission. */
    p_ctrl->p_reg->SCR0 = preserved_scr0 &
                          (uint32_t) ~(R_SCI_B0_SCR0_TE_Msk | R_SCI_B0_SCR0_RE_Msk | R_SCI_B0_SCR0_RIE_Msk);
    p_ctrl->p_tx_src = NULL;

    /* Apply new baud rate register settings. */
    p_ctrl->p_reg->SCR2 = ((sci_b_baud_setting_t *) p_baud_setting)->baudrate_bits_u.baudrate_bits;
...
fsp_err_t R_SCI_B_UART_Abort (uart_ctrl_t * const p_api_ctrl, uart_dir_t communication_to_abort)
{
    ...
    if (UART_DIR_TX & communication_to_abort)
    {
        ...
        /* Make sure no transmission is in progress. Setting SCR0_b.TE to 0 when SSR_b.TEND is 0 causes SCI peripheral
         * to work abnormally. */
        FSP_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->SSR_b.TEND, 1U);
         ...
         * internally after rewriting the control register. */
 #if BSP_FEATURE_SCI_HAS_SISSR
        FSP_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->SISSR_b.ITEF, 0U);
 #else
        FSP_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->SCR0_b.TE, 0U);
                                      ...
                                      sci_b_baud_setting_t * const p_baud_setting)
{
    ...
    p_baud_setting->baudrate_bits_u.baudrate_bits_b.brr  = SCI_B_UART_BRR_MAX;
    p_baud_setting->baudrate_bits_u.baudrate_bits_b.brme = 0U;
    p_baud_setting->baudrate_bits_u.baudrate_bits_b.mddr = SCI_B_UART_MDDR_MIN;
        ...
        for (uint32_t i = 0U; i < SCI_B_UART_NUM_DIVISORS_ASYNC; i++)
        {
            ...
            if (temp_brr <= (SCI_B_UART_BRR_MAX + 1U))
            {
                while (temp_brr > 0U)
                {
                    ...
                    {
                        p_baud_setting->baudrate_bits_u.baudrate_bits_b.bgdm  = g_async_baud[i].bgdm;
                        p_baud_setting->baudrate_bits_u.baudrate_bits_b.abcs  = g_async_baud[i].abcs;
                        p_baud_setting->baudrate_bits_u.baudrate_bits_b.abcse = g_async_baud[i].abcse;
                        p_baud_setting->baudrate_bits_u.baudrate_bits_b.cks   = g_async_baud[i].cks;
                        p_baud_setting->baudrate_bits_u.baudrate_bits_b.brr   = (uint8_t) temp_brr;
                        hit_bit_err = bit_err;
                        hit_mddr    = mddr;
                    }
                    ...
                    if (bitrate_modulation)
                    {
                        p_baud_setting->baudrate_bits_u.baudrate_bits_b.brme = 1U;
                        p_baud_setting->baudrate_bits_u.baudrate_bits_b.mddr = hit_mddr;
                    }
                    else
                    {
                        break;
                                                  ...
                                                  uint32_t                           sci_buffer_address)
{
    ...
    p_info->transfer_settings_u.transfer_settings_word_b.size = TRANSFER_SIZE_1_BYTE;

    if (UART_DATA_BITS_9 == p_ctrl->p_cfg->data_bits)
    {
        p_info->transfer_settings_u.transfer_settings_word_b.size = TRANSFER_SIZE_2_BYTE;
    }
...
static fsp_err_t r_sci_b_uart_transfer_open (sci_b_uart_instance_ctrl_t * const p_ctrl, uart_cfg_t const * const p_cfg)
{
    ...
    if (NULL != p_cfg->p_transfer_rx)
    {
        ...
        p_info->transfer_settings_u.transfer_settings_word = SCI_B_UART_DTC_RX_TRANSFER_SETTINGS;
    ...
    if (NULL != p_cfg->p_transfer_tx)
    {
        ...
        p_info->transfer_settings_u.transfer_settings_word = SCI_B_UART_DTC_TX_TRANSFER_SETTINGS;
...
static void r_sci_b_uart_config_set (sci_b_uart_instance_ctrl_t * const p_ctrl, uart_cfg_t const * const p_cfg)
{
    ...
    uint32_t scr3 = (uint32_t) R_SCI_B0_SCR3_DDIR_Msk;
    scr3 |= ((uint32_t) p_cfg->data_bits << SCI_B_UART_SCR3_CHAR_OFFSET) & SCI_B_UART_SCR3_CHAR_MASK;
    scr3 |= ((uint32_t) p_cfg->stop_bits << R_SCI_B0_SCR3_STOP_Pos) & R_SCI_B0_SCR3_STOP_Msk;
    scr3 |= ((uint32_t) p_extend->rx_edge_start << R_SCI_B0_SCR3_RXDESEL_Pos) & R_SCI_B0_SCR3_RXDESEL_Msk;
    scr3 |= ((uint32_t) p_extend->rs485_setting.enable << R_SCI_B0_SCR3_DEEN_Pos) & R_SCI_B0_SCR3_DEEN_Msk;
    scr3 |= ((uint32_t) p_extend->clock << SCI_B_UART_SCR3_CKE_OFFSET) & SCI_B_UART_SCR3_CKE_MASK;
#if SCI_B_UART_CFG_FIFO_SUPPORT
    if (p_ctrl->fifo_depth > 0U)
    {
        scr3 |= (1U << R_SCI_B0_SCR3_FM_Pos) & R_SCI_B0_SCR3_FM_Msk;
    }
#endif
    ...
    if ((SCI_B_UART_CLOCK_EXT8X == p_extend->clock) || (SCI_B_UART_CLOCK_EXT16X == p_extend->clock))
    {
        ...
        scr2 = SCI_B_UART_SCR2_DEFAULT_VALUE;

        if (SCI_B_UART_CLOCK_EXT8X == p_extend->clock)
        {
            /* Set baud rate as (external clock / 8) */
            scr2 |= 1U << R_SCI_B0_SCR2_ABCS_Pos;
        }
    }
    else
    {
        /* Set the baud rate settings for the internal baud rate generator. */
        scr2 |= p_baud_setting->baudrate_bits_u.baudrate_bits;
    }

    p_ctrl->p_reg->SCR2 = scr2;

    /* Configure flow control pin. */
    uint32_t scr1 = ((uint32_t) (p_extend->flow_control << R_SCI_B0_SCR1_CTSE_Pos) & SCI_B_UART_SCR1_FLOW_CTSRTS_MASK);

    /* TXD pin is at high level when TE is 0. */
    scr1 |= (3U << SCI_B_UART_SCR1_SPB2_OFFSET) & SCI_B_UART_SCR1_SPB2_MASK;

    if (0 != p_cfg->parity)
    {
        scr1 |=
            (((UART_PARITY_EVEN ==
               p_cfg->parity) ? 1U : 3U) << SCI_B_UART_SCR1_PARITY_OFFSET) & SCI_B_UART_SCR1_PARITY_MASK;
    }

    scr1 |= ((uint32_t) p_extend->noise_cancel << R_SCI_B0_SCR1_NFEN_Pos) &
            R_SCI_B0_SCR1_NFEN_Msk;
    p_ctrl->p_reg->SCR1 = scr1;

    p_ctrl->p_reg->SCR4 = 0U;
    ...
    uint32_t decr = ((uint32_t) (p_extend->rs485_setting.polarity << R_SCI_B0_DECR_DELVL_Pos)) &
                    R_SCI_B0_DECR_DELVL_Msk;
    decr |= ((uint32_t) p_extend->rs485_setting.assertion_time << R_SCI_B0_DECR_DESU_Pos) &
            R_SCI_B0_DECR_DESU_Msk;
    decr |= ((uint32_t) p_extend->rs485_setting.negation_time << R_SCI_B0_DECR_DEHLD_Pos) &
            R_SCI_B0_DECR_DEHLD_Msk;
    p_ctrl->p_reg->DECR = decr;
}
...
static void r_sci_b_irq_cfg (sci_b_uart_instance_ctrl_t * const p_ctrl, uint8_t const ipl, IRQn_Type const irq)
{
    ...
    R_BSP_IrqCfg(irq, (uint32_t) ipl, p_ctrl);
}
...
void sci_b_uart_txi_isr (void)
{
    ...
    if ((NULL == p_ctrl->p_cfg->p_transfer_tx) && (0U != p_ctrl->tx_src_bytes))
    {
        ...
        if (0U != p_ctrl->fifo_depth)
        {
            uint32_t fifo_count = p_ctrl->p_reg->TFSR_b.T;
            ...
            /* Clear TDRE flag */
            p_ctrl->p_reg->SSCR |= SCI_B_UART_SSCR_TDREC_MASK;
        }
        else
 #endif
        {
...
void sci_b_uart_rxi_isr (void)
{
 ...
 #endif
    {
 ...
 #else
        {
            ...
            else
            {
                ...
                /* Point to LSB of data in Big Endian */
                memcpy((void *) p_ctrl->p_rx_dest,
                       (void const *) ((uint8_t *) &data + (sizeof(uint32_t) - p_ctrl->data_bytes)),
                       (size_t) p_ctrl->data_bytes);
 #else
                memcpy((void *) p_ctrl->p_rx_dest, (void const *) &data, (size_t) p_ctrl->data_bytes);
 #endif
 ...
 #if SCI_B_UART_CFG_FIFO_SUPPORT
        } while ((p_ctrl->fifo_depth > 0U) && ((p_ctrl->p_reg->RFSR_b.R) > 0U));

        if (p_ctrl->fifo_depth > 0U)
        {
            p_ctrl->p_reg->SSCR |= SCI_B_UART_SSCR_RDRFC_MASK;
        }
...
void sci_b_uart_tei_isr (void)
{
    ...
    p_ctrl->p_reg->SCR0 &= (uint32_t) ~(R_SCI_B0_SCR0_TE_Msk | R_SCI_B0_SCR0_TIE_Msk | R_SCI_B0_SCR0_TEIE_Msk);
...
void sci_b_uart_eri_isr (void)
{
    ...
    uint32_t     ssr   = p_ctrl->p_reg->SSR;
    uart_event_t event = (uart_event_t) ((((ssr & R_SCI_B0_SSR_ORER_Msk) == R_SCI_B0_SSR_ORER_Msk) << 5U) |
                                         (((ssr & R_SCI_B0_SSR_AFER_Msk) == R_SCI_B0_SSR_AFER_Msk) << 4U) |
                                         (((ssr & R_SCI_B0_SSR_APER_Msk) == R_SCI_B0_SSR_APER_Msk) << 3U));

    /* Check if there is a break detected. */
    if ((UART_EVENT_ERR_FRAMING == (event & UART_EVENT_ERR_FRAMING)) && (0U == p_ctrl->p_reg->SSR_b.RXDMON))
    {
        event |= UART_EVENT_BREAK_DETECT;
    }

    /* Clear error condition. */
    p_ctrl->p_reg->SSCR |= SCI_B_UART_RCVR_ERR_MASK;
