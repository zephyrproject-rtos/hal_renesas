 ...
 **********************************************************************************************************************/
 ...
 #pragma weak fsp_error_log
 #pragma weak bsp_init          = bsp_init_internal
#elif defined(__CCRX__)
 #define WEAK_ERROR_ATTRIBUTE    __attribute__((weak))
 #define WEAK_INIT_ATTRIBUTE     __attribute__((weak))
...
#elif defined(__CCRX__)
 #define FSP_SECTION_VERSION     ".fsp_version"
#endif
...
BSP_ATTRIBUTE_KEEP BSP_DONT_REMOVE const uint8_t g_fsp_version_string[] BSP_PLACE_IN_SECTION(FSP_SECTION_VERSION) =
    FSP_VERSION_STRING;

/* Unique FSP version ID. */
BSP_ATTRIBUTE_KEEP BSP_DONT_REMOVE const uint8_t g_fsp_version_build_string[] BSP_PLACE_IN_SECTION(FSP_SECTION_VERSION)
    =
...
/* The default assert implementation for CCRX and GCC brings in printing/formatting code.  FSP overrides the default assert
 * behavior to reduce code size. */

 #if !BSP_CFG_USE_STANDARD_ASSERT

/*******************************************************************************************************************//**
 * Default implementation of assert for CCRX and GCC.
 **********************************************************************************************************************/
...
#if (defined __CCRX__)
 #if defined(_RX_ORDINAL)
BSP_WEAK_REFERENCE long write (long fileno, const unsigned char * buf, long count)
{
    FSP_PARAMETER_NOT_USED(fileno);
    FSP_PARAMETER_NOT_USED(buf);

    return count;
}

BSP_WEAK_REFERENCE long read (long fileno, unsigned char * buf, long count)
{
    FSP_PARAMETER_NOT_USED(fileno);
    FSP_PARAMETER_NOT_USED(buf);
...
BSP_WEAK_REFERENCE long lseek (long fileno, long offset, long base)
{
    FSP_PARAMETER_NOT_USED(fileno);
    FSP_PARAMETER_NOT_USED(offset);
    FSP_PARAMETER_NOT_USED(base);
...
BSP_WEAK_REFERENCE void abort (void)
{
    while (1)
    {
        /* assert() failure trap */
...
BSP_WEAK_REFERENCE void exit (long status)
{
    FSP_PARAMETER_NOT_USED(status);

    while (1)
    {
        /* Program termination trap */
    }
}
