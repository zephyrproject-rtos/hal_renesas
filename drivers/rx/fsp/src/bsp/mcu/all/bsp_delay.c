...
void R_BSP_SoftwareDelay (uint32_t delay, bsp_delay_units_t units)
{
    ...
    else
    {
         ...
         * loop of the delay_loop takes 5 cycles which at 32768 Hz is 155 us. That does not include the overhead below prior to even getting
         * to the delay loop. Given this, at this frequency anything less then a delay request of 155 us will not even generate a single
...
{
    /* The loop takes 5 cycles for RXv1, 4 cycles for RXv2 and 3 cycles for RXv3.
     * NOP and SUB instructions take 1 cycle, and BNE instructions take 3 cycles, but the execution cycle of the pipeline differs depending on the CPU. */
    __asm volatile ("bra.b loop          \n"
                    "nop                 \n"
                    "loop:               \n"
                    "nop                 \n"
                    "sub #01H, r1        \n"
